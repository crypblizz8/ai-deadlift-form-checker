  ...
  return (
    <div className="space-y-6">
      ...
      <div className="text-center">
        ...
        <div className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${getScoreColor(analysis.overallScore)}`}>
            ...
            <div key={key} className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-2">
                <h5 className="font-medium text-gray-900 capitalize">
                  {key === 'liftOff' ? 'Lift Off' : key === 'midRange' ? 'Mid Range' : key === 'barPath' ? 'Bar Path' : key}
                </h5>
                <div className="flex items-center space-x-2">
                  <span className="text-sm font-medium text-gray-700">{point.score}/100</span>
                  <div className={`w-2 h-2 rounded-full ${getScoreColorBar(point.score)}`}></div>
                </div>
              </div>
              ...
              <div className="w-full bg-gray-200 rounded-full h-2 mb-3">
                <div 
                  className={`h-2 rounded-full transition-all duration-500 ${getScoreColorBar(point.score)}`}
              ...
              {'strengths' in point && point.strengths && point.strengths.length > 0 && (
                <div className="mb-2">
              ...
              {'improvements' in point && point.improvements && point.improvements.length > 0 && (
                <div>
                  <h6 className="text-xs font-medium text-orange-700 mb-1">Areas for Improvement:</h6>
        ...
        <div className="grid gap-4">
          ...
          {analysis.practiceAreas && analysis.practiceAreas.length > 0 && (
            <div className="border border-gray-200 rounded-lg p-4">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <h5 className="font-medium text-gray-900">Practice Focus Areas</h5>
              </div>
          ...
          {analysis.safetyNotes && analysis.safetyNotes.length > 0 && (
            <div className="border border-red-200 bg-red-50 rounded-lg p-4">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <h5 className="font-medium text-red-900">Safety Considerations</h5>
              </div>
          ...
          {(!analysis.practiceAreas || analysis.practiceAreas.length === 0) && (
            <div className="border border-gray-200 rounded-lg p-4">
              ...
              <ul className="text-sm text-gray-600 space-y-1">
                <li>• Work on maintaining a neutral spine throughout the lift</li>
                <li>• Practice the hip hinge movement pattern with lighter weights</li>
                <li>• Focus on keeping the bar close to your body during the ascent</li>
              </ul>
            </div>
          )}
          ...
          <div className="border border-gray-200 rounded-lg p-4">
            ...
            <p className="text-sm text-gray-600">
              Record another video after practicing these techniques to see your improvement over time.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="text-center pt-4 border-t border-gray-200">
        <p className="text-xs text-gray-500">
          Analysis completed on {new Date(analysis.timestamp).toLocaleDateString()} at {new Date(analysis.timestamp).toLocaleTimeString()}
        </p>
      </div>
    </div>
  )
